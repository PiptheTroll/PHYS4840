...
def demo_audio_processing():
    ...
    # Load the audio file
    fs, audio_data = wavfile.read('/d/users/iank/Downloads/audio.wav')
    ...
    #filtered2 = np.fft.ifft(X_filtered)

    #wavfile.write('audio_filtered.wav', fs, np.real(filtered2).astype(np.float32))
...
def compare_speeds():
    ...
    for N in sizes:
        ...
        #print(x)
        #print(len(x))
        times_dft.append(lb.dft(x)[1])
        times_radix2.append(lb.fft_ct(x)[1])
        times_bluestein.append(lb.fft_bluestein(x)[1])
        times_zeropad.append(lb.fft_zeropad(x)[1])
        times_numpy.append(np_fft_time(x)[1])

    fig, ax = plt.subplots()
    
    ax.plot(np.array(sizes), times_dft, label='discrete')
    ax.plot(np.array(sizes), times_radix2, label='radix')
    ax.plot(np.array(sizes), times_bluestein, label='bluestein')
    ax.plot(np.array(sizes), times_zeropad, label='zeropad')
    ax.plot(np.array(sizes), times_numpy, label='numpy')

    ax.set_xlabel('length of the input')
